...
def identity_block(input_tensor, kernel_size, filters, stage, block):
    ...
    x = Conv2D(filters1, (1, 1), use_bias = False, kernel_regularizer=regularizers.l2(0.0005), name=conv_name_base + '2a')(input_tensor)
    x = BatchNormalization(axis=bn_axis, epsilon = 1e-5, momentum=0.999, name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)

    x = Conv2D(filters2, kernel_size,
               padding='same', use_bias = False, kernel_regularizer=regularizers.l2(0.0005), name=conv_name_base + '2b')(x)
    x = BatchNormalization(axis=bn_axis,  epsilon = 1e-5, momentum=0.999, name=bn_name_base + '2b')(x)
    x = Activation('relu')(x)

    x = Conv2D(filters3, (1, 1), use_bias = False, kernel_regularizer=regularizers.l2(0.0005),  name=conv_name_base + '2c')(x)
...
def conv_block(input_tensor, kernel_size, filters, stage, block, strides=(2, 2)):
    ...
    x = Conv2D(filters1, (1, 1), strides=strides, use_bias = False, kernel_regularizer=regularizers.l2(0.0005),
               name=conv_name_base + '2a')(input_tensor)
    x = BatchNormalization(axis=bn_axis,  epsilon = 1e-5, momentum=0.999, name=bn_name_base + '2a')(x)
    x = Activation('relu')(x)

    x = Conv2D(filters2, kernel_size, padding='same', use_bias = False, kernel_regularizer=regularizers.l2(0.0005),
    ...
    shortcut = Conv2D(filters3, (1, 1), strides=strides, use_bias = False, kernel_regularizer=regularizers.l2(0.0005),
...
def ResNet101(include_top=True, weights='imagenet',
    ...
    x = ZeroPadding2D((3, 3))(img_input)
    x = Conv2D(64, (7, 7), strides=(2, 2), name='conv1', use_bias = False, kernel_regularizer=regularizers.l2(0.0005) )(x)
    x = BatchNormalization(axis=bn_axis,  epsilon = 1e-5, momentum=0.999, name='bn_conv1')(x)
    x = Activation('relu')(x)
    x = MaxPooling2D((3, 3), strides=(2, 2), padding = 'same')(x)
